...
const DetailRow = ({ label, value }) => value ? (
  <div style={{ display:'flex', justifyContent:'space-between', padding:'0.65rem 0',
    ...
    <span style={{ color:'#ececec', fontSize:'0.88rem', fontWeight:500,
...
export default function ListingDetail() {
  ...
  if (error) return (
    <div style={{ minHeight:'100vh', background:'#0d0d0d' }}>
      ...
      <div style={{ textAlign:'center', padding:'6rem 1.5rem', color:'#666' }}>
        ...
        <p>{error}</p>
        <Link to="/listings" style={{ color:'#c41e2a', marginTop:'1rem', display:'inline-block' }}>
          ← Back to listings
        </Link>
      </div>
    </div>
  ...
  const inp = { width:'100%', background:'#1a1a1a', border:'1px solid #2a2a2a', borderRadius:6,
    padding:'0.65rem 0.9rem', color:'#ececec', fontSize:'0.88rem', outline:'none',
      ...
      <main style={{ maxWidth:1200, margin:'0 auto', width:'100%', padding:'2rem 1.5rem', flex:1 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:'2rem', alignItems:'start' }}
          ...
          <div>
                ...
                {images.map((img, i) => (
                  <div key={i} onClick={() => setMainImg(i)}
                    style={{ width:72, height:52, borderRadius:6, overflow:'hidden', cursor:'pointer',
                      border:`2px solid ${i===mainImg?'#c41e2a':'#2a2a2a'}`, flexShrink:0 }}>
            ...
            {/* Title + price */}
            <h1 style={{ color:'#ececec', fontSize:'1.5rem', fontWeight:800, marginBottom:'0.4rem' }}>
              {listing.title}
            </h1>
            <div style={{ color:'#c41e2a', fontSize:'1.7rem', fontWeight:800, marginBottom:'1.5rem' }}>
            ...
            <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:10,
              ...
              <h3 style={{ color:'#ececec', fontSize:'0.95rem', fontWeight:700,
            ...
            {listing.description && (
              <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:10, padding:'1.25rem' }}>
                <h3 style={{ color:'#ececec', fontSize:'0.95rem', fontWeight:700, marginBottom:'0.75rem' }}>
                  Description
                </h3>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'0.9rem', marginBottom:'1rem' }}>
                  {dealer.logo
                    ...
                    : <div style={{ width:52, height:52, borderRadius:'50%', background:'#1e1e1e',
                        ...
                        color:'#c41e2a', fontWeight:800, fontSize:'1.1rem', border:'2px solid #2a2a2a',
                        flexShrink:0 }}>
                        {dealer.businessName?.[0]}
                      </div>
                  }
                  <div>
                    <div style={{ color:'#ececec', fontWeight:700, fontSize:'0.95rem', display:'flex',
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
                  {dealer.phone && (
                    <a href={`tel:${dealer.phone}`} style={{ display:'flex', alignItems:'center', gap:8,
                      ...
                      padding:'0.6rem 0.9rem', color:'#ececec', fontSize:'0.85rem', textDecoration:'none' }}>
                      📞 {dealer.phone}
                    </a>
                  )}
                  {dealer.whatsapp && (
            ...
            {/* Enquiry form */}
            <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:10, padding:'1.25rem' }}>
              <h3 style={{ color:'#ececec', fontSize:'0.95rem', fontWeight:700, marginBottom:'1rem' }}>
                Send Enquiry
              </h3>
              <form onSubmit={handleSubmit(onEnquiry)} noValidate>
                ...
                {user && (
                  <div style={{ background:'#1a1a1a', borderRadius:6, padding:'0.6rem 0.9rem',
                    ...
                    Sending as <span style={{ color:'#ececec' }}>{user.name}</span>
                  </div>
                )}
            ...
            <div style={{ display:'flex', gap:'1rem' }}>
              <div style={{ flex:1, background:'#141414', border:'1px solid #2a2a2a', borderRadius:8,
                ...
                <div style={{ color:'#c41e2a', fontWeight:700, fontSize:'1.1rem' }}>{listing.views || 0}</div>
                <div style={{ color:'#555', fontSize:'0.72rem' }}>Views</div>
              </div>
              <div style={{ flex:1, background:'#141414', border:'1px solid #2a2a2a', borderRadius:8,
                padding:'0.75rem', textAlign:'center' }}>
                <div style={{ color:'#c41e2a', fontWeight:700, fontSize:'1.1rem' }}>{listing.enquiryCount || 0}</div>
