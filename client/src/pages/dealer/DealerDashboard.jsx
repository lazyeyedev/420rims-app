...
const StatCard = ({ label, value, accent }) => (
  <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:10,
    ...
    <div style={{ color: accent ? '#c41e2a' : '#666', fontSize:'0.75rem',
      fontWeight:700, textTransform:'uppercase', letterSpacing:0.5, marginBottom:8 }}>
      {label}
    </div>
    <div style={{ color:'#ececec', fontSize:'1.9rem', fontWeight:800 }}>
      {value ?? <span style={{ color:'#333', fontSize:'1.2rem' }}>—</span>}
    </div>
  </div>
);

const statusColors = { new:'#c41e2a', read:'#888', responded:'#52c07a', closed:'#555' };
  ...
  return (
    <DealerLayout>
      ...
      {dealer && !dealer.isApproved && (
        <div style={{ background:'#2a1f00', border:'1px solid #c41e2a44', borderRadius:8,
          ...
          <div>
            ...
            <div style={{ color:'#8b1219', fontSize:'0.82rem', marginTop:2 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.5rem' }}>
        <h1 style={{ color:'#ececec', fontSize:'1.4rem', fontWeight:700 }}>Dashboard</h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'1.5rem', alignItems:'start' }}
        ...
        <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:10, overflow:'hidden' }}>
          <div style={{ padding:'1rem 1.25rem', borderBottom:'1px solid #1e1e1e',
            ...
            <span style={{ color:'#ececec', fontWeight:700, fontSize:'0.92rem' }}>Recent Enquiries</span>
            <Link to="/dealer/enquiries" style={{ color:'#c41e2a', fontSize:'0.8rem' }}>View all</Link>
          </div>
          {loading ? (
        ...
        <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:10,
          ...
          <div style={{ color:'#ececec', fontWeight:700, fontSize:'0.92rem', marginBottom:'0.25rem' }}>
            Quick Actions
          </div>
          {[
            ['/dealer/listings/create', '+ Add New Listing',   '#c41e2a', '#0d0d0d'],
            ['/dealer/listings',        '◧ View All Listings', '#1a1a1a', '#ececec'],
            ['/dealer/profile',         '◉ Edit Profile',      '#1a1a1a', '#ececec'],
